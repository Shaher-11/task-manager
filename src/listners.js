...
const deleteProject = (e) => {
  ...
  if (shownProj === null && projectsList.length > 0) {
    const defaultShownTab = document.querySelector(
      `#proj_${projectsList[0].id}`
    );
...
const cancelEditTask = (e) => {
  ...
  const titleInput = document.querySelector(
    `#edit_task_title_${projId}_${taskId}`
  );
  titleInput.value = task.title;
  const descriptionInput = document.querySelector(
    `#edit_task_description_${projId}_${taskId}`
  );
  descriptionInput.value = task.description;
  const priorityInput = document.querySelector(
    `#edit_task_priority_${projId}_${taskId}`
  );
  priorityInput.value = task.priority;
  const statusInput = document.querySelector(
    `#edit_task_status_${projId}_${taskId}`
  );
  statusInput.value = task.status;

  const divShowMode = document.querySelector(
    `#task_show_mode_${projId}_${taskId}`
  );
  divShowMode.classList.toggle("hide");
  const divEditMode = document.querySelector(
    `#task_edit_mode_${projId}_${taskId}`
  );
...
const submitEditTask = (e) => {
  ...
  const dateInput = document.querySelector(
    `#edit_task_date_${projId}_${taskId}`
  );
  if (dateInput.value !== "") {
  ...
  const statusDisplay = document.querySelector(
    `#display_task_status_${projId}_${taskId}`
  );
...
const displayTask = (task) => {
  ...
  priorityBox.setAttribute(
    "id",
    `display_task_priority_${task.projId}_${task.id}`
  );
  ...
  spanDescription.setAttribute(
    "id",
    `display_task_description_${task.projId}_${task.id}`
  );
  spanDescription.textContent = task.description;

  const spanStatus = document.createElement("span");
  spanStatus.setAttribute(
    "id",
    `display_task_status_${task.projId}_${task.id}`
  );
  ...
  editTaskTiteLabel.setAttribute(
    "for",
    `edit_task_title_${task.projId}_${task.id}`
  );
  ...
  editTaskTitleInput.setAttribute(
    "id",
    `edit_task_title_${task.projId}_${task.id}`
  );
  ...
  editTaskDescriptionLabel.setAttribute(
    "for",
    `edit_task_description_${task.projId}_${task.id}`
  );
  ...
  editTaskDescriptionInput.setAttribute(
    "id",
    `edit_task_description_${task.projId}_${task.id}`
  );
  editTaskDescriptionInput.setAttribute("value", task.description);

  editTaskDescriptionDiv.append(
  ...
  editPriorityLabel.setAttribute(
    "for",
    `edit_task_priority_${task.projId}_${task.id}`
  );
  const editPrioritySelect = document.createElement("select");
  editPrioritySelect.setAttribute(
    "id",
    `edit_task_priority_${task.projId}_${task.id}`
  );
  ...
  editStatusLabel.setAttribute(
    "for",
    `edit_task_status_${task.projId}_${task.id}`
  );
  const editStatusSelect = document.createElement("select");
  editStatusSelect.setAttribute(
    "id",
    `edit_task_status_${task.projId}_${task.id}`
  );
...
const taskNav = (myproject) => {
  ...
  headerAddTask.addEventListener("click", (e) => addTask(e)); //addTask?
  ...
  addTaskDescriptionLabel.setAttribute(
    "for",
    `task_description_${myproject.id}`
  );
  ...
  addTaskDescriptionInput.setAttribute(
    "id",
    `task_description_${myproject.id}`
  );
  addTaskDescriptionInput.setAttribute(
    "placeholder",
    "Type a description for your task here!"
  );

  addTaskDescriptionDiv.append(
    addTaskDescriptionLabel,
